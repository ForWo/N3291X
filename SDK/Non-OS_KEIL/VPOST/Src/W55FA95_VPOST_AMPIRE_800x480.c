...
INT vpostLCMInit_AMPIRE_800x480(PLCDFORMATEX plcdformatex, UINT32 *pFramebuf)
{
	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {800,480,800};	
//	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {10,66,(UINT8)210};

#ifdef __KEYPAD_MxN__
	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {10,00,(UINT8)0x58};
#else
//	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {10,00,(UINT8)0x89};
//	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {0x64,0x2f,(UINT8)0x00};	
	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {0x63,0x2e,(UINT8)0xff};		
#endif
...
//#define OPT_24BIT_MODE
	...
	outpw(REG_AHBIPRST, inpw(REG_AHBIPRST) & ~VPOSTRST);	
		...
		else if (u32PLLclk == 324)	// 32.4 = 324/2/5
		{		
			u32Clkin = 1;
			u32ClockDivider = 4;
		}			
...
//	u32ClockDivider = u32PLLclk / 40000;
	u32ClockDivider = u32PLLclk / 50000;	
	...
	outpw(REG_GPDFUN, inpw(REG_GPDFUN) & ~0x003C0000);		// disable HSYNC/VSYNC/VDEN pins		
...
#ifdef __KEYPAD_MxN__	
	// set keypad register
	outpw(REG_LCM_TCON1, (inpw(REG_LCM_TCON1) & ~0x00FF0000) | 0x3A0000);
...
//	outp32(REG_MISC_DS_GPD, 0x00000E00);
	outp32(REG_MISC_DS_GPB, 0x00000000);
	outp32(REG_MISC_DS_GPC, 0x00000000);
	outp32(REG_MISC_DS_GPD, 0x00000000);

//	BacklightControl(TRUE);			
	return 0;
}

INT32 vpostLCMDeinit_AMPIRE_800x480(VOID)
{
	vpostVAStopTrigger();
	vpostFreeVABuffer();
