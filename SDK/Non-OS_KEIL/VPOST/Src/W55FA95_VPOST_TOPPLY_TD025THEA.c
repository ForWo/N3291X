 ...
 * 
 * FILENAME
 *     W55FA95_VPOST_TOPPLY_TD025THEA.c
 *
 * VERSION
 *     0.1 
 *
...
#if defined(HAVE_TOPPLY_TD025THEA)
...
void ILI_I9322_CCIR601_720Y_NTSC_INIT(void)
{
	return;
...
//	ILITek_WtReg(0x06,0xBF);		// Bit[7:4] = 0xB --> CCIR656 640x480
//	ILITek_WtReg(0x06,0xBC);		// Bit[7:4] = 0xB --> CCIR656 640x480	
	ILITek_WtReg(0x06,0x8C);		// Bit[7:4] = 0x7 --> CCIR601 720x480	
									// Bit[3:2] = 0x3 --> auto detection mode for NTSC/PAL
									// Bit[1:1] = 0x1 --> up-to-down scan
									// Bit[0:0] = 0x1 --> left-to-right scan										
...
INT vpostLCMInit_TOPPLY_TD025THEA(PLCDFORMATEX plcdformatex, UINT32 *pFramebuf)
{

	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {1, 0xEF, 0x24};	// Horizontal direction is decreased by 1
	volatile S_DRVVPOST_SYNCLCM_VTIMING sVTiming = {1,1,1};			// Vertical direction is not decreased by 1
	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {720*2,480, 720};	
...
INT32 vpostLCMDeinit_TOPPLY_TD025THEA(VOID)
{
	vpostVAStopTrigger();
	vpostFreeVABuffer();
