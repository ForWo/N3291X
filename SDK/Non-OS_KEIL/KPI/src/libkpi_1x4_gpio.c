...
static unsigned int readkey(void)
{
	...
	read0 = (read0 & 0x18) >> 3;
	gpio_readport(GPIO_PORTG, (unsigned short *)&read1);
	read1 = (read1 & 0x04) | ((read1>>1) & 0x08);
	read0 |= read1;	
	if(read0 == 0x0F)
		return(0);
...
void kpi_init(void)
{
	...
	// PORTA[3,4]
	gpio_setportdir(GPIO_PORTA, ((1 << 3) | (1 << 4)), 0);
	gpio_setportpull(GPIO_PORTA, ((1 << 3) | (1 << 4)), ((1 << 3) | (1 << 4)));	
	gpio_setintmode(GPIO_PORTA, ((1 << 3) | (1 << 4)), 
					((1 << 3) | (1 << 4)), ((1 << 3) | (1 << 4)));

	//PORTG[2,4]
	gpio_configure(GPIO_PORTG, 2);
	gpio_configure(GPIO_PORTG, 4);
	gpio_setportdir(GPIO_PORTG, ((1 << 2) | (1 << 4)), 0);
	gpio_setportpull(GPIO_PORTG, ((1 << 2) | (1 << 4)), 0);	
	gpio_setintmode(GPIO_PORTG, ((1 << 2) | (1 << 4)), 
					((1 << 2) | (1 << 4)), ((1 << 2) | (1 << 4)));
...
int kpi_open(unsigned int src)
{
	...
	gpio_setsrcgrp(GPIO_PORTA, ((1 << 3) | (1 << 4)), src);
	gpio_setsrcgrp(GPIO_PORTG, ((1 << 2) | (1 << 4)), src);
