...
*/
extern void usbSetFeature(void);
void RemoteWakup_Test(void)
{
	...
	{	
		...
		outp32(REG_HC_RH_PORT_STATUS1, 0x4); 						//Port suspend
...
void PortSuspendResume_Test(void)
{
	...
	{		
		...
		outp32(REG_HC_RH_PORT_STATUS1, inp32(REG_HC_RH_PORT_STATUS1) | BIT3);	//Write 1 for resume port
		 while( (inp32(REG_HC_RH_PORT_STATUS1)&BIT18)==0)
		 {		 	
...
INT IntegrateTest(void)
{
	...
	sysprintf("[4]. USB camera [W99683] test using 1022 packet size												\n");
	sysprintf("[5]. USB camera [W99683] test using 768 packet size												\n");
	sysprintf("[6]. USB camera [W99683] test using 512 packet size												\n");
	sysprintf("[7]. USB camera [W99683] overnight test (1022) packet size											\n");
	sysprintf("[8]. Integration test [W99683+Keyboard+Disk]														\n");
	sysprintf("[9]. Integration test [W99683+Disk under a hub that connect to port 1 and port 0 connect a mouse]				\n");
	sysprintf("[A]. Port suspend, system power down, remote wake-up by HID mouse									\n");					
	sysprintf("[B]. Port suspend, system power down, wake-up by gpio then resume device								\n");	
	sysprintf("=============================================================================================\n");
	u32Item = sysGetChar();
	switch(u32Item)
	{
		case '1': 		
    		...
    		case '3': 	    
				...
				USBKeyboardInit();				
    				...
    				break;    
    		case '4':    		
    				Isochronous_TestPacketSize(1022);
    				break;	
    		case '5':
    				Isochronous_TestPacketSize(768);
    				break;
    		case '6':
    				Isochronous_TestPacketSize(512);
    				break;
    		case '7':
    				while(1)
    					Isochronous_TestPacketSize(1022);
    		...
    		case '9':			
				...
				DeInitUsbSystem();			
    				...
    				break;								   				
    		case 'A':	USBKeyboardInit();	
    				RemoteWakup_Test();
    				break;	
    		case 'B':	PortSuspendResume_Test();
    				break;				
...
INT main()
{
...
#if 1	//USBH's clock source needs multiple of 48MHz
	...
	do
	{
		...
		sysprintf("[1] GPB0 and GPB1		(FPGA J78)																				\n");
		sysprintf("[2] GPA10 and GPA11	(Conflict with UART 1)																		\n");
		sysprintf("[3] Disable																										\n");
		sysprintf("============================================================================================\n");
		u32Item = sysGetChar();
		switch(u32Item)
		{
			case '1': 	u32UsbhPort1 = HOST_LIKE_PORT1_0;
					//USB_PortInit(u32UsbhPort0, HOST_LIKE_PORT1_0);		
					break;
	    		case '2': 	u32UsbhPort1 = HOST_LIKE_PORT1_1;
	    				//USB_PortInit(u32UsbhPort0, HOST_LIKE_PORT1_1);		
	    				break;
	    		case '3':   u32UsbhPort1 = HOST_LIKE_PORT1_DISABLE;
	    				//USB_PortInit(u32UsbhPort0, HOST_LIKE_PORT1_DISABLE);		
	    				break;		
		}
	}while((u32Item> '3'));		
	...
	{
		sysprintf("============================================================================================\n");
		sysprintf("Please select the USB host port 0 through GPIO														       					\n");
		sysprintf("[1] GPD3 and GPD4		(FPGA-J20)																				\n");
		sysprintf("[2] ND4 and ND5		(FPGA-J32)																				\n");
		sysprintf("[3] GPD14 and GPD15	(FPGA-U4... Designer gaurantee it work			)												\n");
		sysprintf("[4] GPA5 and GPA6		(FPGA-J12)																				\n");
		sysprintf("[5] Disable																										\n");
		sysprintf("============================================================================================\n");
		u32Item = sysGetChar();
		switch(u32Item)
		{
			case '1': 	
					USB_PortInit(u32UsbhPort1, HOST_LIKE_PORT0_0);			
					break;
	    		case '2': 		
	    				USB_PortInit(u32UsbhPort1, HOST_LIKE_PORT0_1);	
	    				break;
	    		case '3': 
	    				USB_PortInit(u32UsbhPort1, HOST_LIKE_PORT0_2);	
	    				break;	
	    		case '4':	
	    				USB_PortInit(u32UsbhPort1, HOST_LIKE_PORT0_3);	
	    				break;	    		
	    		case '5':		
	    				USB_PortInit(u32UsbhPort1, HOST_LIKE_PORT0_DISABLE);	
	    				break;	    				
		}
	}while((u32Item> '5'));	
