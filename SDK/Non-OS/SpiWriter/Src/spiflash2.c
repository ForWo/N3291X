...
{
	int volatile i,j;
	UINT32 u32Tmp,u32Count,count,page;	
	PUINT32 u32Buf;
	...
	for (i=0; i<count; i++)
	{
		if(!g_4byte_adderss)
		{
			if((addr+i*256) & 0xFF000000)
			{
				Enter4ByteMode();
				g_4byte_adderss = TRUE;
				PRINTF("SPI 4 Byte Address Mode Enable - 0x%X (R)\n", addr + i*256 );	
			}	
		}	
		else
		{
			if(!((addr+i*256) & 0xFF000000))
			{
				...
				PRINTF("SPI 4 Byte Address Mode Disable - 0x%X (R)\n", addr + i*256 );	
			}			
		}		
...
int usiEraseSector(UINT32 addr, UINT32 secCount)
{
	...
	for (i=0; i<secCount; i++)
	{
		if(!g_4byte_adderss)
		{
			if((addr+i*64*1024) & 0xFF000000)
			{
				Enter4ByteMode();
				g_4byte_adderss = TRUE;
				PRINTF("SPI 4 Byte Address Mode Enable(E)\n");	
			}	
		}	
		else
		{
	...
	for (i=0; i<count; i++)
	{
		...
		if(!g_4byte_adderss)
		{
			if((addr+i*256)& 0xFF000000)
			{
				Enter4ByteMode();
				g_4byte_adderss = TRUE;
				PRINTF("SPI 4 Byte Address Mode Enable - 0x%X (W)\n", addr + i*256 );	
			}	
		}	
		else
		{
			if(!((addr+i*256)& 0xFF000000))
			{
				...
				Reset();
				g_4byte_adderss = FALSE;
				PRINTF("SPI 4 Byte Address Mode Disable - 0x%X (W)\n", addr + i*256 );		
			}			
		}	
	...
	while (len > 0)
	{
		...
		if(!g_4byte_adderss)
		{
			if(addr & 0xFF000000)
			{
				Enter4ByteMode();
				g_4byte_adderss = TRUE;
				PRINTF("SPI 4 Byte Address Mode Enable(W-SST)\n");	
			}	
		}	
		else
		{
			if(!(addr & 0xFF000000))
			{
				...
				Reset();
				g_4byte_adderss = FALSE;
				PRINTF("SPI 4 Byte Address Mode Disable(W-SST)\n");	
			}			
		}	
		usiWriteEnable();
