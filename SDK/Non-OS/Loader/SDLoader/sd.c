...
INT fmiSD_Init()
{
	...
	else
	{
		...
		if (i == 2)		// MMC memory
		{
			...
			_fmi_uR3_CMD = 1;
			if (fmiSDCmdAndRsp(1, 0x80ff8000, 4000) != 2)	// MMC memory
			{
				resp = inpw(REG_SDRSP0);
				while (!(resp & 0x00800000))		// check if card is ready
				{
					_fmi_uR3_CMD = 1;
					fmiSDCmdAndRsp(1, 0x80ff8000, 0);		// high voltage
					resp = inpw(REG_SDRSP0);
				}
	...
	if (pSD->uCardType == FMI_TYPE_SD_HIGH)
		printf("This is high capacity SD memory card\n");
	if (pSD->uCardType == FMI_TYPE_SD_LOW)
		printf("This is standard capacity SD memory card\n");
	if (pSD->uCardType == FMI_TYPE_MMC)
		printf("This is MMC memory card\n");
#endif
...
INT fmiSelectCard()
{
	...
	{
		outpw(REG_SDCR, inpw(REG_SDCR) & 0xffff7fff);
	}
...
INT fmiSD_Write_in(UINT32 uSector, UINT32 uBufcnt, UINT32 uSAddr)
{
	...
	for (i=0; i<loop; i++)
	{
		...
		{
#ifdef DEBUG
			printf("1. fmiSD_Write:write data error [0x%x]\n", inpw(REG_SDISR));
#endif
			outpw(REG_SDISR, 0x02);
			return -1;
		}
	...
	if (loop != 0)
	{
		...
		{
#ifdef DEBUG
			printf("2. fmiSD_Write:write data error [0x%x]!\n", inpw(REG_SDISR));
#endif
			outpw(REG_SDISR, 0x02);
			return -1;
		}
