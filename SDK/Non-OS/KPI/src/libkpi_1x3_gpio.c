...
static unsigned int readkey(void)
{
	...
	read0 = ((read0 & 0x04)>>2) | ((read0>>3) & 0x06);
...
void kpi_init(void)
{
	...
	gpio_configure(GPIO_PORTG, 5);
	gpio_setportdir(GPIO_PORTG, ((1 << 2) | (1 << 4) | (1 << 5)), 0);
	gpio_setportpull(GPIO_PORTG, ((1 << 2) | (1 << 4) | (1 << 5)), 0);	
	gpio_setintmode(GPIO_PORTG, ((1 << 2) | (1 << 4) | (1 << 5)), 
...
int kpi_open(unsigned int src)
{
	...
	gpio_setsrcgrp(GPIO_PORTG, ((1 << 2) | (1 << 4) | (1 << 5)), src);
